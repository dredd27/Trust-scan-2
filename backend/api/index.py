...
async def analyze_with_ai(text: Optional[str] = None, image_base64: Optional[str] = None) -> dict:
    ...
    try:
        ...
        return {
            "extracted_text": None,
            "ai_analysis": "Servizio AI temporaneamente non disponibile.",
            "risk_indicators": []
        }
...
@app.post("/api/calculate-risk", response_model=RiskResult)
async def calculate_risk(request: CalculateRiskRequest):
    ...
    if score <= 3:
        ...
        level = "ALTO"
        label = "ALTA PROBABILITA DI TRUFFA"
...
@app.get("/api/analyses", response_model=List[RiskResult])
